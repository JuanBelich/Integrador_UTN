...
SubProceso recorrerPalabra(palabra,letra Por Referencia) //buscamos saber si la letra que ingresa el usuario está dentro de la palabra secreta
	...
	FinPara
	si j=0 Entonces
		Escribir "La letra no esta en la palabra";
	FinSi
FinSubProceso
...
Funcion vof <- retorno(palabra,letra) //buscamos saber si la letra que ingresa el usuario está dentro de la palabra secreta
	...
	Para i<-0 Hasta (Longitud(palabra)-1) con paso 1 hacer
		j <- 0;;
		si Subcadena(palabra,i,i) = letra Entonces
			j <- 1; //esta es la parte positiva donde si encuentra la letra 
			suma <- suma + j;
		FinSi 
...
Proceso juego
	...
	Definir num,random,pos,intentos,i,j,tipeado,moneda,a,intento Como Entero;
		...
		segun (num) hacer
				...
				4:
					...
					Repetir
						...
						leer letra;
						h <- falso; //la j tiene que estar en cero cuando el ciclo vuelva a inicializarse para que no salten errores explicar 
